...
export const roleHasAccess = (role, resource, action) => {
  ...
  // If resource and action don't exist in the access map, return false
  if (!RESOURCE_ACCESS_MAP[resource] || !RESOURCE_ACCESS_MAP[resource][action]) {
    return false;
  }
  
  // Get the roles that have access to this resource and action
  const allowedRoles = RESOURCE_ACCESS_MAP[resource][action];
  
  // If the role is directly allowed, return true
  if (allowedRoles.includes(role)) {
    return true;
  }
  
  // Check if the role is higher in the hierarchy than any of the allowed roles
  ...
  // Check if any of the allowed roles are lower in the hierarchy
  return allowedRoles.some(allowedRole => {
    const allowedRoleIndex = ROLE_HIERARCHY.indexOf(allowedRole);
    return allowedRoleIndex !== -1 && roleIndex > allowedRoleIndex;
  });
};

/**
 * Check if any of the user's roles has access to a resource and action
 * 
 * @param {Array} userRoles - Array of user role objects
 * @param {string} resource - The resource to check access for
 * @param {string} action - The action to check access for
 * @returns {boolean} - True if any role has access to the resource and action
 */
export const userRolesHaveAccess = (userRoles, resource, action) => {
  ...
  return userRoles.some(role => {
    if (!role || !role.code) return false;
    return roleHasAccess(role.code, resource, action);
  });
};
 ...
 * 
 * @param {Array} userRoles - Array of user role objects
 * @returns {Array} - Combined array of unique permissions from all roles
 */
export const getPermissionsForRoles = (userRoles) => {
  ...
  const allPermissions = userRoles.reduce((permissions, role) => {
    if (!role || !role.code) return permissions;
    
    const rolePermissions = getPermissionsForRole(role.code);
