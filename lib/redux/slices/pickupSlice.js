...
export const createPickup = createAsyncThunk(
  ...
  async (data, { rejectWithValue }) => {
    try {
      // Handle empty kenekId - convert to null
      if (data.kenekId === "none" || data.kenekId === "") {
        data.kenekId = null;
      }
...
export const updatePickupStatus = createAsyncThunk(
  ...
  async ({ id, status, notes }, { getState, rejectWithValue }) => {
    try {
      ...
      const currentPickup = getState().pickup.pickups.find(p => p._id === id) ||
                          getState().pickup.pickup;
      ...
      // Validate status transition
      if (!isValidStatusTransition(currentPickup.status, status)) {
        throw new Error(`Tidak dapat mengubah status dari ${currentPickup.status} ke ${status}`);
      }
      ...
      const response = await pickupAPI.updateStatus(id, payload);
      return response.data;
    } catch (error) {
      return rejectWithValue(
        error.response?.data?.message || error.message || "Gagal mengubah status pengambilan"
      );
    }
  }
);
...
// Initial state with better organized structure
const initialState = {
  pickups: [],
  pickup: null,
  loading: false,
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchPickupById.fulfilled, (state, action) => {
        state.loading = false;
        state.pickup = action.payload.data;
      })
      ...
      .addCase(createPickup.fulfilled, (state, action) => {
        state.loading = false;
        state.success = true;
        // Add to pickups array if it exists
        if (state.pickups) {
      ...
      .addCase(updatePickup.fulfilled, (state, action) => {
        ...
        // Update pickup in state if exists
        if (state.pickup && state.pickup._id === action.payload.data._id) {
          state.pickup = action.payload.data;
        }

        // Update in pickups list if exists
        if (state.pickups && state.pickups.length > 0) {
          state.pickups = state.pickups.map((pickup) =>
            pickup._id === action.payload.data._id
              ? action.payload.data
              : pickup
          );
        }
      ...
      .addCase(deletePickup.fulfilled, (state, action) => {
        ...
        if (state.pickups && state.pickups.length > 0) {
          state.pickups = state.pickups.filter(
            (pickup) => pickup._id !== action.payload
          );
        }

        // Clear current pickup if it's the one deleted
...
export const { clearPickup, clearError, clearSuccess, resetPickupState } =
  pickupSlice.actions;
