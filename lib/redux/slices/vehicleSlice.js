...
export const updateVehicle = createAsyncThunk(
  ...
  async ({ id, vehicleData }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || "Gagal memperbarui data kendaraan"
      );
    }
  }
);
...
export const uploadVehicleDocument = createAsyncThunk(
  ...
  async ({ id, documentType, formData }, { rejectWithValue }) => {
    try {
      const response = await vehicleAPI.uploadDocument(
        id,
        formData,
        documentType
      );
      ...
      return {
        ...
        data: normalized.data,
        documentUrl:
          normalized.data.documentUrl || normalized.data.data?.documentUrl,
      };
    } catch (error) {
      return rejectWithValue(
    ...
    builder
      ...
      .addCase(fetchDeliveryVehicles.fulfilled, (state, action) => {
        ...
        if (action.payload && action.payload.data) {
          state.deliveryVehicles = Array.isArray(action.payload.data) 
            ? action.payload.data 
            : [];
          console.log("Delivery vehicles loaded:", state.deliveryVehicles.length);
        } else if (Array.isArray(action.payload)) {
          state.deliveryVehicles = action.payload;
          console.log("Delivery vehicles loaded (array):", state.deliveryVehicles.length);
        } else {
          console.warn("Unexpected delivery vehicles response format:", action.payload);
      ...
      .addCase(fetchDeliveryVehicles.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload || action.error.message;
        console.error("Failed to load delivery vehicles:", state.error);
        state.deliveryVehicles = []; // Clear the array on error
      })
      ...
      .addCase(deleteVehicle.fulfilled, (state, action) => {
        ...
        if (
          ...
          state.currentVehicle._id === action.payload
        ) {
          state.currentVehicle = null;
        }
        state.error = null;
      ...
      .addCase(uploadVehiclePhoto.fulfilled, (state, action) => {
        ...
        if (
          state.currentVehicle &&
          state.currentVehicle._id === action.payload.id
        ) {
          if (
            action.payload.photoType === "driver" ||
            action.payload.photoType === "driverPhoto"
          ) {
            state.currentVehicle.fotoSupir = action.payload.photoUrl;
          } else if (
            action.payload.photoType === "helper" ||
            action.payload.photoType === "helperPhoto"
          ) {
