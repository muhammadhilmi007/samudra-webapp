...
export const fetchCustomerSTTs = createAsyncThunk(
  ...
  async (customerId, { rejectWithValue }) => {
    try {
      const response = await customerAPI.getSTTs(customerId)
      return { customerId, data: response.data }
    } catch (error) {
      return rejectWithValue(
        error.response?.data?.message || 
        'Gagal mengambil data STT pelanggan'
      )
    }
  }
)
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchCustomersByBranch.fulfilled, (state, action) => {
        state.loading = false
        state.customersByBranch[action.payload.branchId] = action.payload.data.data
        state.error = null
      })
      ...
      .addCase(fetchCustomerSTTs.fulfilled, (state, action) => {
        state.loading = false
        state.customerSTTs[action.payload.customerId] = action.payload.data.data
        state.error = null
      })
      ...
      .addCase(fetchCustomerCollections.fulfilled, (state, action) => {
        state.loading = false
        state.customerCollections[action.payload.customerId] = action.payload.data.data
        state.error = null
      })
      ...
      .addCase(fetchCustomerPickups.fulfilled, (state, action) => {
        state.loading = false
        state.customerPickups[action.payload.customerId] = action.payload.data.data
        state.error = null
      })
