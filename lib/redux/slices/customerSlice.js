...
export const fetchCustomerSTTs = createAsyncThunk(
  ...
  async (customerId, thunkAPI) => {
    try {
      // Use the correct API call from the sttAPI module
      const response = await sttAPI.getBySender(customerId);
      return { customerId, data: response.data };
    } catch (error) {
      return thunkAPI.rejectWithValue(
        error.response?.data?.message || "Failed to fetch customer STTs"
      );
    }
  }
);
...
export const updateCustomer = createAsyncThunk(
  ...
  async ({ id, customerData }, { rejectWithValue, dispatch }) => {
    try {
      ...
      console.log("Update data:", normalizedData);
      console.log("Branch ID being sent:", normalizedData.cabangId);
      
      const response = await customerAPI.update(id, normalizedData);
    ...
    builder
      ...
      .addCase(fetchCustomers.fulfilled, (state, action) => {
        ...
        state.customers = customersData.map(customer => ({
          ...
          cabangId: customer.cabangId?._id?.toString() || customer.cabangId?.toString()
        }));
      ...
      .addCase(fetchCustomerPickups.fulfilled, (state, action) => {
        state.loading = false;
        state.customerPickups[action.payload.customerId] =
          action.payload.data.data;
        state.error = null;
      })
      ...
      // Also update the fulfilled case for createCustomer
      .addCase(createCustomer.fulfilled, (state, action) => {
        state.loading = false;
        // Normalize the customer data
        const normalizedCustomer = {
          ...action.payload.data,
          _id: action.payload.data._id?.toString(),
          cabangId: action.payload.data.cabangId?.toString(),
        };
      ...
      .addCase(updateCustomer.fulfilled, (state, action) => {
        ...
        const normalizedCustomer = {
          ...
          cabangId: updatedCustomer.cabangId?.toString()
        };
        
        console.log("Normalized updated customer:", normalizedCustomer);
        
        // Update the customer in the customers array
        state.customers = state.customers.map((customer) => {
          if (customer._id === normalizedCustomer._id) {
            console.log("Updating customer in state:", customer._id);
