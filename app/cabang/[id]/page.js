...
function EditCabangContent({ params }) {
  ...
  const { currentBranch, loading, error, success } = useSelector(
    (state) => state.cabang
  );
  ...
  }, [dispatch, branchId, toast]);

  useEffect(() => {
    if (currentBranch) {
      setFormData({
        namaCabang: currentBranch.namaCabang || "",
        divisiId: currentBranch.divisiId || "",
        alamat: currentBranch.alamat || "",
        kelurahan: currentBranch.kelurahan || "",
        kecamatan: currentBranch.kecamatan || "",
        kota: currentBranch.kota || "",
        provinsi: currentBranch.provinsi || "",
        kontakPenanggungJawab: {
          nama: currentBranch.kontakPenanggungJawab?.nama || "",
          telepon: currentBranch.kontakPenanggungJawab?.telepon || "",
          email: currentBranch.kontakPenanggungJawab?.email || "",
        },
      });
    }
  }, [currentBranch]);
  ...
  const handleSubmit = async (e) => {
    ...
    console.log("Updating branch data:", formattedData);
    await dispatch(updateBranch({ 
      id: branchId, 
      branchData: formattedData 
    }));
  };
        ...
        <main className="flex-1 overflow-y-auto bg-gray-50 p-4 sm:p-6">
          <div className="mx-auto max-w-1xl space-y-6">
            <div className="flex items-center">
              ...
              <h1 className="text-2xl font-bold tracking-tight">Edit Cabang</h1>
            </div>

            <Card>
              <CardHeader>
                <CardTitle>Formulir Edit Cabang</CardTitle>
              </CardHeader>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        value={formData.divisiId ? formData.divisiId.toString() : ""}
                        onValueChange={(value) => {
                          setFormData({
                            ...formData,
                            divisiId: value,
                      ...
                      {formErrors.divisiId && (
                        <p className="text-red-500 text-sm mt-1">{formErrors.divisiId}</p>
                      )}
                    </div>
