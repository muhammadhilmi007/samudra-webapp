...
export default function EditPegawaiPage({ params }) {
  ...
  const { currentEmployee, loading, error, success } = useSelector((state) => state.pegawai)
  const { roles, loading: rolesLoading } = useSelector((state) => state.pegawai)
  ...
  useEffect(() => {
    if (currentEmployee) {
      setFormData({
        ...
        aktif: currentEmployee.aktif !== undefined ? currentEmployee.aktif : true
      })
  ...
  const handleFileChange = async (e, type) => {
    ...
    try {
      if (type === 'fotoProfil') {
        ...
        await dispatch(uploadDocument({ id: employeeId, docType: 'ktp', formData }))
      }
      
      toast({
        title: 'Berhasil',
        description: `${type === 'fotoProfil' ? 'Foto profil' : 'Dokumen KTP'} berhasil diunggah`,
        variant: 'success',
      })
    } catch (error) {
      toast({
        title: 'Error',
        description: `Gagal mengunggah ${type === 'fotoProfil' ? 'foto profil' : 'dokumen KTP'}`,
        variant: 'destructive',
      })
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await dispatch(updateEmployee({ id: employeeId, formData: formDataToSend }))
    } catch (error) {
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <div className="px-6">
              <TabsList className="grid w-full grid-cols-3">
                <TabsTrigger value="info-dasar">Informasi Dasar</TabsTrigger>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={handleInputChange}
                      className={formErrors.jabatan ? 'border-red-500' : ''}
                    />
                    {formErrors.jabatan && (
                      <p className="text-sm text-red-500">{formErrors.jabatan}</p>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      <SelectTrigger 
                        id="roleId"
                        className={formErrors.roleId ? 'border-red-500' : ''}
                      >
                    ...
                    {formErrors.roleId && (
                      <p className="text-sm text-red-500">{formErrors.roleId}</p>
                    )}
                  </div>
              ...
              <TabsContent value="login" className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    <Label 
                      htmlFor="cabangId" 
                      className={formErrors.cabangId ? 'text-red-500' : ''}
                    >
                      Cabang <span className="text-red-500">*</span>
                    </Label>
                    <Select
                      value={formData.cabangId}
                      onValueChange={(value) => handleSelectChange('cabangId', value)}
                    >
                      <SelectTrigger 
                        id="cabangId"
                        className={formErrors.cabangId ? 'border-red-500' : ''}
                      >
                    ...
                    {formErrors.cabangId && (
                      <p className="text-sm text-red-500">{formErrors.cabangId}</p>
                    )}
                  </div>
                  ...
                  <div className="space-y-2 md:col-span-2">
                    <div className="flex items-center space-x-2">
                      <Checkbox 
                        ...
                        onCheckedChange={(checked) => setShowPasswordField(checked)}
                      />
                      <Label htmlFor="changePassword">
                        Ubah Password
                      </Label>
                    </div>
                    <p className="text-sm text-gray-500">Centang jika ingin mengubah password pegawai</p>
                  </div>
                  ...
                  {showPasswordField && (
                    <div className="space-y-2 md:col-span-2">
                      <Label 
                        htmlFor="password" 
                        className={formErrors.password ? 'text-red-500' : ''}
                      >
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      <Checkbox 
                        ...
                        onCheckedChange={(checked) => handleCheckboxChange('aktif', checked)}
                      />
                      <Label htmlFor="aktif">
                        Pegawai Aktif
                      </Label>
                    </div>
                    <p className="text-sm text-gray-500">Centang jika pegawai dalam status aktif</p>
                  </div>
                </div>
